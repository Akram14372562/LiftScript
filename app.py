...
def get_proxy_setting():
    proxy_req = request.form.get('proxy_toggle', 'off')
    if proxy_req == 'on':
        try:
...
def index():
    ...
    if not submit_action or submit_action == "Back To Dashboard":
        ...
        return refresh_token(ol, number)
...
def process_single_card(ol, number, access_token, session_id, data, count1):
    ...
    # Get proxy setting from the form
    ...
    # Get tokens - Include proxy parameter
    ...
    try:
        ...
        # Check if we got an error from tokens API
        if 'error' in result:
        ...
        if all([stripe_token, first_data_token, braintree_token]):
            # Submit card - Include proxy parameter
